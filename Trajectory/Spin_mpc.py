...
import sys,os
from iris_env.dexhand.allegro_spin.spin_object import AllegroObject,RolloutVmapWrapper,get_obs
...
CTRL_RANGE = 0.25
class Spin_MPC(object):
    def __init__(self, obj_name, Horizon=40, rollout_length=150, planner_num=256, sigma=0.4) -> None:
        self.Horizon = Horizon
        self.rollout_length = rollout_length
        self.planner_num = planner_num
        self.sigma = sigma

        self.init_pos_dict = {'cube': np.array([-0.011549, -0.0224281, 0.101794]), 'bunny': np.array([0.01, 0.0, 0.05])}

        self.param = env_param()
        self.param.init_object_pos = self.init_pos_dict[obj_name]
        self.param.object_name = obj_name
        self.env = AllegroObject(self.param,frame_skip=4, timestep = 0.005)#0.005
        # jit everything
    ...
    def generate_traj_allegro(self,torch_model,render = False, init_pos = None, init_vel = None, noiselevel = 0.0, lam_mppi = 1.0, filename = 'simulation_output.html'):
        ...
        for t in range(self.rollout_length):
            ...
            obj_pos = obs[16:19]
            obj_quat = obs[19:23]
            obj_mat = R.from_quat(obj_quat).as_matrix()

            obj_ang_vel = obj_mat @ obs[42:45]
            obj_z = obs[18]
            ...
            if t%10 == 9:
                ...
                print('t',t,'ang vel',obj_ang_vel,'xyz', obj_pos, 'd', np.array(dist))
                #print('a', action)
    ...
    def generate_random_traj(self,init_pos = None, init_vel = None):
        ...
        for t in range(self.rollout_length):
            ...
            if t%10 == 9:
                print('t',t,'q',obj_q,'xyz', obj_pos, 'd', np.array(dist))
                #print('a', action)
                #print('a lowlvl', action_lowlevel)
            #print('action',action)
            env_obss.append(obs)
