...
def discounted_sum(tensor, discount_factor):
  """
  Calculates the discounted sum of a tensor.

  Args:
    tensor: A 1D PyTorch tensor.
    discount_factor: The discount factor (a float between 0 and 1).

  Returns:
    A 1D PyTorch tensor with the discounted sum.
  """
  
  weights = torch.pow(discount_factor, torch.arange(len(tensor), device=tensor.device))
  weighted_tensor = tensor * weights
  return torch.sum(weighted_tensor)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--err', help='mean error number of every batch',type=int)
    parser.add_argument('--disc', help='discount factor',type=float)
    ...
    while round_cnt < NUM_ROUNDS:
        ...
        for p in current_pairs_list:
            true_labels_list.append(torch_reward_gt(p[0]).sum() > torch_reward_gt(p[1]).sum())
            # do a flip
            rew_0 = torch_reward_gt(p[0]).flatten()
            rew_1 = torch_reward_gt(p[1]).flatten()

            pref_label = discounted_sum(rew_0,args.disc) > discounted_sum(rew_1,args.disc)
        ...
        err_num = 0
        correct_num = GROUP_LENGTH - args.err

        for i in range(GROUP_LENGTH):
            if current_labels_list[i] != true_labels_list[i]:
                err_num+=1
                
        if err_num>0:
            print('There is {} wrong labels'.format(err_num))
            print("current",trajs_label)
            print("true",true_labels_list)
        else:
