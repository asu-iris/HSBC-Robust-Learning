...
def torch_reward_gt_2(input): #input:(x,u)
    robot_height = input[...,0]/10
    ...
    pads_z_front = input[...,17:19]/10
    pads_z_rear = input[...,19:21]/10
    foot_cost =  torch.sum((pads_z_front - target_foot_height) ** 2,dim=-1) + torch.sum(pads_z_rear ** 2,dim=-1) 
...
def input_wrapper(input,mask):
    input_reduced = input[...,mask]
    input_reduced[...,0]*=10
    input_reduced[...,17:21]*=10
