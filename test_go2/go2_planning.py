...
ens.stacked_params = torch.load('./Data/Go2/RA/error_0/run_0/ensemble_0.pt')
print(ens.stacked_params['fc_0.weight'].shape)

mpc = Unitree_MPC(planner_num=1024,rollout_length=300,Horizon=25,sigma=1.0)

model_fn = lambda x:ens.prediction(x).mean(dim=0)
obs_seq, act_seq = mpc.generate_traj_unitree(model_fn,render=True,init_pos=None, init_vel=None, noiselevel=0.0,lam_mppi=0.01)
