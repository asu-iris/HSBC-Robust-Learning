...
class Hopper(PipelineEnv):
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def step(self, state: State, action: jax.Array) -> State:
        ...
        # IRIS added
        tilt_cost = (pipeline_state.q[2])**2
        fall_cost = (z-1.25)**2
        vertical_cost=(z-pipeline_state.x.pos[3,2])**2
        obs = self._get_obs(pipeline_state)
        reward = 0.0005*forward_reward + 0.0*healthy_reward - \
            ctrl_cost-10*tilt_cost - 0*fall_cost+0.1*vertical_cost
