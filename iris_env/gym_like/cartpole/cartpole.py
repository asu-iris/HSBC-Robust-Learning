...
class Cartpole(PipelineEnv):
    ...
    def __init__(self, env_param, frame_skip=10, timestep=0.002,  **kwargs):
        ...
        # load model
        path = os.path.join(filedir,'model/cartpole.xml')
    ...
    def _get_obs(self, pipeline_state: base.State) -> jax.Array:
        ...
        cart_pos = pipeline_state.q[0]
        angle_sin = jnp.sin(pipeline_state.q[1])
        angle_cos = jnp.cos(pipeline_state.q[1])
        positions = jnp.stack((cart_pos,angle_sin,angle_cos),axis = -1)
        obs = jnp.concat((positions,pipeline_state.qd),axis = -1)
        return obs
