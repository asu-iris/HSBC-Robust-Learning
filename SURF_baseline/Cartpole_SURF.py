    ...
    while round_cnt < NUM_ROUNDS:
        ...
        while len(current_pairs_list)<GROUP_LENGTH:
            ...
            for t_2 in traj_buf[::-1]:
                start_idx_1 = np.random.choice(41,2)
                np.random.shuffle(start_idx_1)
                start_idx_2 = np.random.choice(41,2)
                np.random.shuffle(start_idx_2)
                for i in range(2):
                    seg_1 = traj_tensor_1[start_idx_1[i]:start_idx_1[i]+60]
                    seg_2 = t_2[start_idx_2[i]:start_idx_2[i]+60]
        ...
        for i in range(500):
            ...
            aug_segs_0 = TDA(segs_0,target_len=50)
            aug_segs_1 = TDA(segs_1,target_len=50)
            ...
            if history_data_unlabelled.numel()>0:
                ...
                if qualified_idx.sum()>0:
                    ...
                    sample_segs_0 = TDA(sampled_nl_data[:,0,...])
                    sample_segs_1 = TDA(sampled_nl_data[:,1,...])
