...
if __name__ == '__main__':
    ...
    mpc = Cartpole_MPC(planner_num=256,rollout_length=100,sigma=1.0)
    ...
    while round_cnt < NUM_ROUNDS:
        ...
        while len(current_pairs_list)<GROUP_LENGTH:
            ...
            if round_cnt<5:
                ...
                while not success_flag:
                    try: #make sure the planning is successful
                        ...
                        obs_seq, act_seq = mpc.generate_traj_cartpole(model_fn,render=False,init_pos=None, init_vel=None,noiselevel=noise_once,lam_mppi=0.01)
                        success_flag = True
            ...
            for t_2 in traj_buf[::-1]:
                start_idx_1 = np.random.choice(51,2)
                np.random.shuffle(start_idx_1)
                start_idx_2 = np.random.choice(51,2)
        ...
        # RIME preprocess: find trustworthy data and flip some labels
        with torch.no_grad():
            reward_pred = ens.prediction(history_data).squeeze(dim=-1).mean(dim=-1)
            reward_0 = reward_pred[...,0]
            reward_1 = reward_pred[...,1]
            P_pred_0 = BT_prob(reward_0,reward_1,alpha=10.0)
            P_pred_1 = 1 - P_pred_0

            P_pred = torch.stack((P_pred_0,P_pred_1),dim=-1)
            P_pred = P_pred.mean(0)
            P_pred = P_pred.flatten(start_dim=0,end_dim=-2)

            tmp_label = torch.zeros_like(P_pred)
            tmp_label[:,0] = history_label_tensor.flatten()  * 1
            tmp_label[:,1] = 1 - history_label_tensor.flatten()  * 1

            KL_div = (-tmp_label * torch.log(P_pred)).sum(1)
            x = max_trust_KL
            baseline = -np.log(x + 1e-8) + alpha * x
            beta_t = max(beta_min,beta_max - round_cnt / k)
            uncertainty = min(beta_t * std_trust_KL, 3.0)

            trust_sample_bool_index = KL_div < baseline + uncertainty
            trust_samples_KL_div = KL_div[trust_sample_bool_index]
            try:
                max_trust_KL = trust_samples_KL_div.max().cpu().numpy()
                std_trust_KL = trust_samples_KL_div.std().cpu().numpy()
            except Exception as e:
                print(e)
            
            flipping_threshold = -np.log(flipping_tau)
            flipping_sample_bool_index = KL_div > flipping_threshold

            trust_sample_index = np.where(trust_sample_bool_index.cpu().numpy())[0]
            untrust_sample_index = np.where(np.bitwise_not(trust_sample_bool_index.cpu().numpy()))[0]
            flipping_sample_index = np.where(flipping_sample_bool_index.cpu().numpy())[0]

            #flip the label
            tmp_label[flipping_sample_index] = 1.0 - tmp_label[flipping_sample_index]
            training_sample_index = np.concatenate((trust_sample_index,flipping_sample_index))

            print("untrusted:",untrust_sample_index)
            print("RIME Flipped", flipping_sample_index)
        #compute KL divergence
        history_label_tensor
        ...
        for i in range(500):
            ...
            P_pred_training = P_pred[training_sample_index]
            label_training = tmp_label[training_sample_index]

            CE_loss = -(label_training * torch.log(P_pred_training + 1e-5)).sum(dim=-1).mean()
