...
if __name__ == '__main__':
    ...
    parser.add_argument('--disagree_thresh', help='disagree threshold',type=float,default=0.75)
    parser.add_argument('--alg_error', help='error setting in algorithm',type=int,default=0)
    parser.add_argument('--gamma', help='gamma factor in loss function',type=float,default=0.75)
    ...
    NOISE_LEVEL=0.2
    MEASURE_THRESH = args.disagree_thresh
    GROUP_LENGTH=args.batchsize
    round_cnt = 0
    current_pairs_list = []
    current_labels_list = []
    history_groups = []
    ...
    while round_cnt < NUM_ROUNDS:
        ...
        while len(current_pairs_list)<GROUP_LENGTH:
            ...
            for t_2 in traj_buf[::-1]:
                ...
                for i in range(2):
                    ...
                    if MEASURE_THRESH == 0.0:
                        trajs = [seg_1,seg_2]
                        print('No measure threshold',measure)
                        disagreement_log.append(measure)
                        current_pairs_list.append(torch.stack(trajs,dim=0))

                    elif measure > MEASURE_THRESH:
                        trajs = [seg_1,seg_2]
        ...
        err_num = args.err
        alg_err_num = args.alg_error
        correct_num = GROUP_LENGTH - err_num
        alg_correct_num = GROUP_LENGTH - alg_err_num
        if err_num>0:
            print('There is {} wrong labels'.format(err_num))
            print('before flip',trajs_label)
            idx = np.random.choice(GROUP_LENGTH,size=err_num,replace=False)
        ...
        for i in range(500):
            ...
            sigmoid_S_batch = torch.sigmoid(3*(S_batch - alg_correct_num*args.gamma)) #2 1
        ...
        in_flag = torch.all((pref_pred==history_label_tensor).sum(dim=-1) >= alg_correct_num - 1,dim=1)
